    ...
    def force_curve(self, t):
        ...
        if self.GRF.stance_flg == 1 and self.Last_flag == 0:
            ...
            if self.show_index == 1:  # 是否展示参数指令，评价力曲线是否达到期望步态百分比
                FmaX_time = (self.F_start1 + self.F_start) * 100 / self.Flag + 0.15  # 开始时刻，支撑相占比，是否完成支撑助力曲线
        ...
        if self.GRF.stance_flg == 1:  # 支撑相
            ...
            if (t <= self.touch_time + self.upstart_time) and (t >= self.touch_time):  # 刚刚触地预紧
                ...
                t1 = self.uprise_time
                force1 = (4 * self.upforce_max * pow(x,3)) / pow(t1,3) -( 3 * self.upforce_max * pow(x,4) ) / pow(t1,4) + force_pre
                force = 2 * force1 / (self.force_p + 1.0)
                ...
                t1 = self.upfall_time
                force1 = self.upforce_max - (4 * self.upforce_max * pow(x, 3)) / pow(t1, 3) + (3 * self.upforce_max * pow(x, 4)) / pow(t1, 4) + force_pre
                force = 2 * force1 / (self.force_p + 1.0)
