...
class Disturb(Strategy):
    def __init__(self):
        # self.only = False
        super(Disturb, self).__init__()
    ...
    def Mode_Callback(self, mode_stance, mode_fight, mode_other,pos_fight):
        ...
        self.pos = pos_fight
        rospy.loginfo(" Disturb param update --> mode_stance=%d,mode_fight=%d \n\t",mode_stance,mode_fight)
        return
    ...
    def force_curve(self, t):
        ...
        if 1:
            ...
            if (t <= self.touch_time + start_time) and (t >= self.touch_time):  # 刚刚触地预紧
                ...
                t1 = fall_time
                force = force_max - (4 * force_max * pow(x, 3)) / pow(t1, 3) + (3 * force_max * pow(x, 4)) / pow(t1,
                                                                                                                 4) + force_pre
