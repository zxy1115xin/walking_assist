...
class StrategyGroup:
    ...
    #
    #         # 判断触地
    #         if grf_bot > 2000 and self.Last_flagL == 0:
    ...
    #         # 判断蹬地
    #         # 登地
    #         if grf_up > 300 and self.index_L_on == 1:
    ...
    #
    #         # 离地
    #         if grf_up < 300 and self.index_L_on == 1 and self.index_L_off == 1:
    ...
    def GRFL_Callback(self, msg):
        if self.param_flag == 1:
            ...
            if self.GRF_L.all_force > 1700 and self.Last_flagL == 0 and backupl == 1:
                self.footL_up = self.footL_up + 1
            else:
                self.footL_up = 0
            if self.Last_flagL == 0 and self.footL_up == 7:
                self.GRF_L.stance_flg = 1

            # 判断支撑相阶段
            if self.GRF_L.all_force >  1700  and self.Last_flagL == 1:
                self.GRF_L.stance_flg = 1
            ...
            if self.GRF_L.all_force <  1700  and self.Last_flagL == 1:
                ...
                self.footL_down = 0
            if self.Last_flagL == 1 and self.footL_down == 7:
                self.GRF_L.stance_flg = 0
    ...
    def GRFR_Callback(self, msg):
        if self.param_flag == 1:
            ...
            if self.GRF_R.all_force > 400 and self.Last_flagR == 0 :
                self.footR_up = self.footR_up + 1
            else:
                self.footR_up = 0


            if self.Last_flagR == 0 and self.footR_up == 5:
                self.GRF_R.stance_flg = 1

            # 持续
            if self.GRF_R.all_force > 400 and self.Last_flagR == 1:
                self.GRF_R.stance_flg = 1
            ...
            if self.GRF_R.all_force < 400 and self.Last_flagR == 1:
                ...
                self.footR_down = 0
            if self.Last_flagR == 1 and self.footR_down == 5:
                self.GRF_R.stance_flg = 0
    ...
    def ParamCallback(self, config, level):  # level为参数的掩码（，表征到底哪个参数是否被改变
        ...
        F_rise = config.F_rise
        F_fall = config.F_fall
        for strategy in self.strategy_list:
            strategy.ParamCallback(F_max,F_start,F_rise,F_fall)
